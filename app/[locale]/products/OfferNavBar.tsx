...
export default function OfferNavBar({ allOfferCategories }: Params) {
	...
	useEffect(() => {
		...
		const firstOfferSection = document
			.getElementById('offers')
			?.querySelectorAll('section[data-offer-id]')[0] as HTMLDivElement;
		firstOfferSection.style.marginTop = isDesktop
			? `calc(-1 * calc(${ref.current?.offsetHeight}px + var(--space)))`
			: 'unset';
	}, [isDesktop]);
